...
router.get('/server-public', async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to retrieve server public key' });
  }
});
...
router.get('/user/:username', async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to retrieve user public key' });
  }
});

// Get all users' public keys (useful for clients to encrypt messages for multiple recipients)
router.get('/users', authenticate, async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to retrieve users public keys' });
  }
});
